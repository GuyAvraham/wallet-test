...
export default function useConnector(): UseConnectorReturnValue {
  ...
  React.useEffect(() => {
    ...
    if (connector) {
      ...
      connect(
        `wc:${connector.handshakeTopic}?bridge=${connector.bridge}&key=${connector.key}`,
  ...
  const connect = async (uri: string, account: string, chainId: number) => {
    try {
      ...
      if (connector.connected === false) await connector.createSession();

      setConnector(connector);
    } catch (error) {
      alertDialogError(
        "Invalid URI format",
        "The URI format that has been entered is invalid, please, try again!",
        "Okay"
      );
    }
  };
